...
interface Project {
  ...
  name: string
  auditor: string
  buildings: string[] // Changed from building: string to buildings: string[]
  createdAt: string
}
...
export function ProjectsManagement() {
  ...
  const handleAddBuilding = (buildingName: string) => {
    if (buildingName && !formData.buildings.includes(buildingName)) {
      setFormData({ ...formData, buildings: [...formData.buildings, buildingName] })
      setSelectedBuilding("")
    }
  }

  const handleRemoveBuilding = (buildingName: string) => {
    setFormData({ ...formData, buildings: formData.buildings.filter((b) => b !== buildingName) })
  }
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="grid gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      ...
                      {mockBuildings
                        .filter((building) => !formData.buildings.includes(building.name))
                        .map((building) => (
                    ...
                    <Button
                      type="button"
                      onClick={() => handleAddBuilding(selectedBuilding)}
                  ...
                  {formData.buildings.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {formData.buildings.map((building) => (
                        <Badge key={building} variant="secondary" className="flex items-center gap-1">
                          {building}
                          <button
                            type="button"
              ...
              <DialogFooter>
                <Button type="submit" disabled={formData.buildings.length === 0}>
