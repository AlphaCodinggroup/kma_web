...
describe("FacilitiesContent", () => {
  ...
  describe("listing", () => {
    it("renders a row per facility with address, city and created date", () => {
      ...
      expect(within(north).getByText("2026-02-10 09:15")).toBeInTheDocument();
    });
  ...
  describe("create", () => {
    ...
    it("surfaces the mutation error inside the dialog", () => {
      ...
      expect(screen.getByText("facility already exists")).toBeInTheDocument();
    });
  ...
  describe("edit", () => {
    ...
    it("loads legacy notes into the description field", async () => {
      renderContent();
      ...
      expect(screen.getByLabelText("Description")).toHaveValue("Legacy notes");
