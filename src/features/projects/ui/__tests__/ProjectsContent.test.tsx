...
// El debounce tiene sus propias pruebas; acá aislamos el filtrado del listado.
...
describe("ProjectsContent", () => {
  ...
  describe("listing", () => {
    it("renders a row per project with users, facilities, status and created date", () => {
      ...
      expect(within(alpha).getByText("2026-01-15 10:30")).toBeInTheDocument();

      const beta = rowFor("beta Bridge");
      expect(within(beta).getByText("Archived")).toBeInTheDocument();
      expect(within(beta).getByText("2025-06-02 08:05")).toBeInTheDocument();
    });
  ...
  describe("sorting", () => {
    it("cycles the project column through ascending, descending and unsorted", async () => {
      ...
      expect(rowNames()).toEqual(["beta Bridge", "Alpha Tower", "Gamma Yard"]);

      await userEvent.click(screen.getByRole("button", { name: /^Project/ }));
      expect(rowNames()).toEqual(["Alpha Tower", "beta Bridge", "Gamma Yard"]);

      await userEvent.click(screen.getByRole("button", { name: /^Project/ }));
      expect(rowNames()).toEqual(["Gamma Yard", "beta Bridge", "Alpha Tower"]);
    ...
    it("sorts by auditor and falls back to an empty key when there is none", async () => {
      ...
      expect(rowNames()).toEqual(["Gamma Yard", "Alpha Tower", "beta Bridge"]);
    });
  ...
  describe("create", () => {
    ...
    it("enables the lookups once the create dialog is open", () => {
      ...
      expect(usersQuerySpy).toHaveBeenLastCalledWith({ role: "auditor" }, true);
  ...
  describe("edit", () => {
    it("opens the edit dialog prefilled with the selected project", async () => {
      ...
      expect(screen.getByLabelText("Project Name")).toHaveValue("Alpha Tower");
      expect(
