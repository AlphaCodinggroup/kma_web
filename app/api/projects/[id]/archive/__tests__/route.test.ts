...
const url = "http://localhost/api/projects/p-1/archive";
...
describe("POST /api/projects/[id]/archive", () => {
  ...
  it("forwards the bearer token to the archive endpoint", async () => {
    ...
    expect(String(upstream)).toBe(
      "https://api.example.com/api/projects/p-1/archive"
    );
  ...
  it("escapes the id before building the upstream url", async () => {
    ...
    const { POST } = await import("../route");
    await POST(request(url, { method: "POST" }), context("p/1"));

    const [upstream] = fetchMock.mock.calls[0] as unknown as FetchArgs;
    expect(String(upstream)).toBe(
      "https://api.example.com/api/projects/p%2F1/archive"
    );
  });
...
describe("/api/projects/[id]/archive with non JSON upstream responses", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
    cookieStore.value = "token-abc";
