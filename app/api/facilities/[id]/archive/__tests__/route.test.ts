...
const url = "http://localhost/api/facilities/f-1/archive";

describe("POST /api/facilities/[id]/archive", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
  ...
  it("forwards the bearer token to the archive endpoint", async () => {
    const fetchMock = vi.fn(async () => jsonResponse({ status: "archived" }));
    vi.stubGlobal("fetch", fetchMock);
    ...
    await expect(res.json()).resolves.toEqual({ status: "archived" });
    const [upstream, init] = fetchMock.mock.calls[0] as unknown as FetchArgs;
    expect(String(upstream)).toBe(
      "https://api.example.com/api/facilities/f-1/archive"
    );
  ...
  it("does not forward query params from the incoming request", async () => {
    ...
    const { POST } = await import("../route");
    await POST(request(`${url}?secret=x`, { method: "POST" }), context("f-1"));
...
describe("/api/facilities/[id]/archive with non JSON upstream responses", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
    cookieStore.value = "token-abc";
