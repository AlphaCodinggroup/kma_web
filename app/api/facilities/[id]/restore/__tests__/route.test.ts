...
const url = "http://localhost/api/facilities/f-1/restore";
...
describe("POST /api/facilities/[id]/restore", () => {
  ...
  it("forwards the bearer token to the restore endpoint without a body", async () => {
    ...
    expect(String(upstream)).toBe(
      "https://api.example.com/api/facilities/f-1/restore"
    );
  ...
  it("escapes the id when building the upstream url", async () => {
    ...
    const { POST } = await import("../route");
    await POST(request(url, { method: "POST" }), context("f-1/../projects"));
