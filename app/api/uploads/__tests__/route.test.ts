...
function postRequest(body: string = JSON.stringify(payload)) {
  return request("http://localhost/api/uploads", {
    method: "POST",
    body,
    headers: { "content-type": "application/json" },
  });
...
describe("POST /api/uploads", () => {
  ...
  it("forwards the bearer token and the body to the uploads endpoint", async () => {
    ...
    expect(String(upstream)).toBe("https://api.example.com/api/uploads");
  ...
  it("does not forward query params from the incoming request", async () => {
    ...
    const { POST } = await import("../route");
    await POST(
      request("http://localhost/api/uploads?secret=x", {
        method: "POST",
...
describe("/api/uploads with non JSON upstream responses", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
    cookieStore.value = "token-abc";
