...
const url = "http://localhost/api/reports/a-1";
...
describe("GET /api/reports/[auditId]", () => {
  ...
  it("forwards the bearer token to the report endpoint", async () => {
  ...
  it("does not forward query params from the incoming request", async () => {
    ...
    const { GET } = await import("../route");
    await GET(request(`${url}?secret=x`), context("a-1"));
