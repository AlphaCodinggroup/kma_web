...
describe("GET /api/reports", () => {
  ...
  it("forwards the bearer token to the reports endpoint", async () => {
  ...
  it("forwards only the declared query params", async () => {
    ...
    await GET(
      request(
        "http://localhost/api/reports?project_id=p-1&user_id=u-1&status=ready&limit=10&last_eval_id=k1&secret=x&projectId=p-2"
      )
    );
