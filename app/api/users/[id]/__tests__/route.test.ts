...
/** Contexto de ruta dinámica con el id del usuario. */
function context(id: string) {
  return { params: Promise.resolve({ id }) };
}

const url = "http://localhost/api/users/u-1";
...
describe("PATCH /api/users/[id]", () => {
  ...
  it("forwards the bearer token and the body as an upstream PUT", async () => {
    ...
    expect(String(upstream)).toBe("https://api.example.com/api/users/u-1");
  ...
  it("escapes the id before building the upstream url", async () => {
    ...
    const { PATCH } = await import("../route");
    await PATCH(patchRequest(), context("u 1/2"));

    const [upstream] = fetchMock.mock.calls[0] as unknown as FetchArgs;
    expect(String(upstream)).toBe("https://api.example.com/api/users/u%201%2F2");
  });
  ...
  it("does not forward query params from the incoming request", async () => {
    ...
    const { PATCH } = await import("../route");
    await PATCH(patchRequest(`${url}?secret=x`), context("u-1"));
  ...
  it("rejects an invalid JSON body with 400 and never reaches the backend", async () => {
    ...
    const { PATCH } = await import("../route");
    const res = await PATCH(patchRequest(url, "no-json"), context("u-1"));
