...
describe("GET /api/users", () => {
  ...
  it("forwards the bearer token to the users endpoint", async () => {
    ...
    expect(String(upstream)).toBe("https://api.example.com/api/users");
...
describe("POST /api/users", () => {
  ...
  it("forwards the bearer token and the body to the users endpoint", async () => {
    const fetchMock = vi.fn(async () => jsonResponse({ id: "u-1" }, 201));
    vi.stubGlobal("fetch", fetchMock);
