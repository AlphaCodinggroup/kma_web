...
/** Contexto de ruta dinámica con el id de la revisión. */
function context(auditReviewId: string) {
  return { params: Promise.resolve({ auditReviewId }) };
}

const url = "http://localhost/api/audits/audit-reviews/r-1";

describe("GET /api/audits/audit-reviews/[auditReviewId]", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
  ...
  it("forwards the bearer token to the audits-review endpoint", async () => {
    const fetchMock = vi.fn(async () => jsonResponse({ id: "r-1" }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("escapes the id before building the upstream url", async () => {
    ...
    const { GET } = await import("../route");
    await GET(request(url), context("r 1/2"));

    const [upstream] = fetchMock.mock.calls[0] as unknown as FetchArgs;
    expect(String(upstream)).toBe(
      "https://api.example.com/api/audits-review/r%201%2F2"
    );
  });
  ...
  it("does not forward query params from the incoming request", async () => {
    ...
    const { GET } = await import("../route");
    await GET(request(`${url}?secret=x`), context("r-1"));
