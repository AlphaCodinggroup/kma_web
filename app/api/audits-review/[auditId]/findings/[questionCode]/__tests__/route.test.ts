...
/** Contexto de ruta dinámica con el id de auditoría y el código de pregunta. */
function context(auditId: string, questionCode: string) {
  return { params: Promise.resolve({ auditId, questionCode }) };
}

const url = "http://localhost/api/audits-review/a-1/findings/q-1";
const payload = { finding: "Missing extinguisher", severity: "high" };
...
describe("PATCH /api/audits-review/[auditId]/findings/[questionCode]", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
  ...
  it("returns 401 and never reaches the backend without a session cookie", async () => {
    ...
    const res = await PATCH(patchRequest(), context("a-1", "q-1"));

    expect(res.status).toBe(401);
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("forwards the bearer token and the body to the findings endpoint", async () => {
    ...
    expect(String(upstream)).toBe(
      "https://api.example.com/api/audits-review/a-1/findings/q-1"
    );
  ...
  it("escapes both path segments before building the upstream url", async () => {
    const fetchMock = vi.fn(async () => jsonResponse({}));
    vi.stubGlobal("fetch", fetchMock);

    const { PATCH } = await import("../route");
    await PATCH(patchRequest(), context("a/1", "q 1"));

    const [upstream] = fetchMock.mock.calls[0] as unknown as FetchArgs;
    expect(String(upstream)).toBe(
      "https://api.example.com/api/audits-review/a%2F1/findings/q%201"
    );
  });
  ...
  it("does not forward query params from the incoming request", async () => {
    ...
    const { PATCH } = await import("../route");
    await PATCH(patchRequest(`${url}?secret=x`), context("a-1", "q-1"));
...
describe("/api/audits-review/[auditId]/findings/[questionCode] with non JSON upstream responses", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
    cookieStore.value = "token-abc";
