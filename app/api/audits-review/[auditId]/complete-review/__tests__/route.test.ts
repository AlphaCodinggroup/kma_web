...
const url = "http://localhost/api/audits-review/a-1/complete-review";

describe("POST /api/audits-review/[auditId]/complete-review", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
  ...
  it("forwards the bearer token to the complete-review endpoint", async () => {
    ...
    expect(String(upstream)).toBe(
      "https://api.example.com/api/audits-review/a-1/complete-review"
    );
  ...
  it("escapes the id before building the upstream url", async () => {
    ...
    expect(String(upstream)).toBe(
      "https://api.example.com/api/audits-review/a%2F1/complete-review"
    );
  });
...
describe("/api/audits-review/[auditId]/complete-review with non JSON upstream responses", () => {
  beforeEach(() => {
    vi.resetModules();
    stubEnv();
    cookieStore.value = "token-abc";
