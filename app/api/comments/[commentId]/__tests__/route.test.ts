...
/** Contexto de ruta dinámica con el id del comentario. */
function context(commentId: string) {
  return { params: Promise.resolve({ commentId }) };
}

const url = "http://localhost/api/comments/c-1";
const payload = { content: "editado" };
...
describe("PUT /api/comments/[commentId]", () => {
  ...
  // El id se valida antes que la sesión, así que un id vacío responde 400
  // incluso sin cookie.
  ...
  it("forwards the bearer token and the body to the comment endpoint", async () => {
    const fetchMock = vi.fn(async () => jsonResponse({ id: "c-1" }));
    vi.stubGlobal("fetch", fetchMock);
    ...
    expect(String(upstream)).toBe("https://api.example.com/api/comments/c-1");
  ...
  it("escapes the id before building the upstream url", async () => {
    ...
    const { PUT } = await import("../route");
    await PUT(putRequest(), context("c/1"));

    const [upstream] = fetchMock.mock.calls[0] as unknown as FetchArgs;
    expect(String(upstream)).toBe("https://api.example.com/api/comments/c%2F1");
  });
  ...
  it("does not forward query params from the incoming request", async () => {
    ...
    const { PUT } = await import("../route");
    await PUT(putRequest(`${url}?secret=x`), context("c-1"));
  ...
  it("rejects an invalid JSON body with 400 and never reaches the backend", async () => {
    ...
    const { PUT } = await import("../route");
    const res = await PUT(putRequest(url, "no-json"), context("c-1"));
